...
/**
 * 主要功能:软键盘管理
 * 隐藏输入法:hideKeyboard;用法页面有edittext,弹出软键盘，这时请求数据要隐藏软件盘
 */
    ...
    public static void closeKeyboard(Activity context){
        try {
            if(null != context){
                ...
                if (null !=imm && imm.isActive() && null !=context.getCurrentFocus()){
                    if(null != context.getCurrentFocus().getWindowToken()){
                        imm.hideSoftInputFromWindow(context.getCurrentFocus().getWindowToken(), InputMethodManager.RESULT_UNCHANGED_SHOWN);
                    }
                }
            }
        }catch (Exception e){}
