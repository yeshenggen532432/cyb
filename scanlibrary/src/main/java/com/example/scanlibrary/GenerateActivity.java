...
public class GenerateActivity extends AppCompatActivity {
    ...
    private void createChineseQRCodeWithLogo() {
        new AsyncTask<Void, Void, Bitmap>() {
            @Override
            protected Bitmap doInBackground(Void... params) {
                Bitmap logoBitmap = BitmapFactory.decodeResource(GenerateActivity.this.getResources(), R.drawable.logo);
    ...
    private void createBarcodeWidthContent() {
        new AsyncTask<Void, Void, Bitmap>() {
            @Override
            protected Bitmap doInBackground(Void... params) {
                int width = BGAQRCodeUtil.dp2px(GenerateActivity.this, 150);
                int height = BGAQRCodeUtil.dp2px(GenerateActivity.this, 70);
                int textSize = BGAQRCodeUtil.sp2px(GenerateActivity.this, 14);
            ...
            protected void onPostExecute(Bitmap bitmap) {
                if (bitmap != null) {
                    ...
                    Toast.makeText(GenerateActivity.this, "生成条底部带文字形码失败", Toast.LENGTH_SHORT).show();
                }
            }
        }.execute();
    }
        ...
        new AsyncTask<Void, Void, Bitmap>() {
            ...
            protected void onPostExecute(Bitmap bitmap) {
                if (bitmap != null) {
                    ...
                    Toast.makeText(GenerateActivity.this, "生成条底部不带文字形码失败", Toast.LENGTH_SHORT).show();
                }
            }
        }.execute();
    }
