 ...
 * 图形验证码:登录
 */
public class MyVerifyCodeView extends View {
    //将图片划分成4*3个小格
    ...
    private boolean isNum = true;//是否是纯数字验证码;false:数字+字符串

    public MyVerifyCodeView(Context context) {
        this(context, null);
    }

    public MyVerifyCodeView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MyVerifyCodeView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            isWrapContent = true;
            mWidth = (int) (textPaint.measureText("0") * 1.5 * codeNum + 20 + MyDisplayUtil.dpToPx(getContext(), 8));
        }
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            mHeight = (int) MyDisplayUtil.dpToPx(getContext(), 40);
        }
    ...
    private void init(AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.MyVerifyCodeView);
        isNetCode = typedArray.getBoolean(R.styleable.MyVerifyCodeView_isNetCode, false);
        codeNum = typedArray.getInteger(R.styleable.MyVerifyCodeView_codeNumber, 4);
        isNum = typedArray.getBoolean(R.styleable.MyVerifyCodeView_isNum, true);
    }
    ...
    private void initView() {
        ...
        bgPaint.setColor(BLUE_BG_COLOR);
        ...
        maxTextSize = MyDisplayUtil.spToPx(getContext(), 50);
        defaultTextSize = MyDisplayUtil.spToPx(getContext(), 30);
        textPaint = new Paint();
    ...
    private void createCodeBitmap() {
        ...
        if (!isWrapContent) {
            ...
            horizontalOffset = (int) ((mWidth - drawWidth - MyDisplayUtil.dpToPx(getContext(), 8)) / 2);
        ...
        //画背景
        myCanvas.drawColor(BLUE_BG_COLOR);
    ...
    private void dynamicSetTextPaint(String text) {
        int avaiWidth = (int) (mWidth * 2.0f / 3 - getPaddingLeft() - getPaddingRight() - MyDisplayUtil.dpToPx(getContext(), 2));
        if (avaiWidth <= 0) {
            return;
        }
