...
public class CCPFlipper extends ViewGroup {
	...
	public void processVelocity(int screenIndex , int duration) {
		...
		if(getScrollX() != index * getWidth()) {
			...
			int _duration = CCPUtil.round(getContext() , 2 * Math.abs(unScrollx)) ;
			if(duration > 0 ) {
				_duration = duration;
			}
	 ...
	 * @param finalIndex
	 */
	public void setScroolIndex(int finalIndex) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if(this.onCCPFlipperMeasureListener != null) {
			this.onCCPFlipperMeasureListener.onCCPFlipperMeasure(widthMeasureSpec, heightMeasureSpec);
		}
