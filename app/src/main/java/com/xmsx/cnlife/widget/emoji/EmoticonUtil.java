...
public class EmoticonUtil
{
	...
	public static ImageGetter getImageGetter(final Context c)
	{
		return new ImageGetter()
		{
			...
			public Drawable getDrawable(String source)
			{
				...
				Integer resID = c.getResources().getIdentifier(source,
						"drawable", c.getPackageName());
	...
	public static void initEmoji()
	{
		...
		String[] stringArray = MyApp.getI().getApplicationContext()
				.getResources().getStringArray(R.array.emoji_code_file);
		if (stringArray != null)
		{
	...
	public static void initEmojiIcons(String[] emojiUnicodes)
	{
		...
		for (String emojiUnicode : emojiUnicodes)
		{
			...
			if (charArray != null && charArray.length > 0)
			{
				...
				if (emojiResid != -1)
				{
					emojiMap.put("[" + emojiUnicode + "]", "emoji_"
							+ emojiResid);
	...
	{
		return getEmoticonResId(num + "", MyApp.getI()
	 ...
	 * 
	 * @param emo
	 *            The local expression of file name for example: emoji_e415.png
	 * @return
	 */
	 ...
	 * @return
	 */
	public static SpannableString emoji2CharSequence(Context context,
                                                     String str, int textSize, boolean isReplaceLine)
	{
	...
	public static boolean containsKeyEmoji(Context context,
                                           SpannableString spannableString, int textSize)
	{
		...
		while (i < charArray.length)
		{
			...
			if (emojiId != -1)
			{
				Drawable drawable = EmoticonUtil.getEmoticonDrawable(context,
						emojiId);

				if (drawable != null)
				{
					drawable.setBounds(0, 0, (int) (1.3D * textSize),
							(int) (1.3D * textSize));
					spannableString.setSpan(new ImageSpan(drawable, 0), i,
	...
	private static String matchEmojiUnicode(String str)
	{
		...
		try
		{
			for (int i = 0; i < charArray.length - 1; i++)
			{
				...
				if ((_index != 55357) || (_index_inc < 56343)
						|| (_index_inc > 57024))
				{
					continue;
				}
