 ...
 * 盘点仓库
 */
public class XStkCheckActivity2 extends XActivity<PxStkCheck> {
    ...
    private int type = 1;//1盘点开单（添加）；2盘点开单（修改）；3多人盘点开单（添加）；4多人盘点开单（修改）
    ...
    private void doIntent() {
        if (billId > 0) {
            getP().queryStkCheck(context, billId, type);
        } else {
            //缓存
    ...
    private void initOther() {
        ...
        findViewById(R.id.layout_add_ware).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                ActivityManager.getInstance().jumpToChooseWareActivity(context, stkId, null);
            }
        });
        //仓库商品
    ...
    StkCheckAdapter mAdapter;
    View footer = null;

    private void initAdapter() {
        //尾部：添加商品
        footer = LayoutInflater.from(context).inflate(R.layout.x_layout_check_storage_footer, null);
//        footer.findViewById(R.id.bottom_add_ware).setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View view) {
//                if (!verifyStkId()) return;//验证仓库
//                Router.newIntent(context)
//                        .putString(ConstantUtils.Intent.TYPE, "6")
//                        .putString(ConstantUtils.Intent.STK_ID, String.valueOf(stkId))
//                        .putParcelableArrayList(ConstantUtils.Intent.CHOOSE_WARE, null)
...
//        });
            ...
            if (RESULT_OK == resultCode && requestCode == ConstantUtils.Intent.REQUEST_CODE_SCAN) {
                if (data != null) {
                    ...
                    if (null != resultList && !resultList.isEmpty()) {
                        doSelectWareList(resultList, true);
                    }
                }
            }
            //选择商品
    ...
    public void refreshAdapter() {
        try{
            ...
            if (dataList != null && dataList.size() >= 2) {
                ...
                if(stkCheckWareBean.getWareId() == null){
                    mAdapter.remove(0);
                }
            }
        ...
        dialog.setOnBtnClickL(null, new OnBtnClickL() {
            ...
            public void onBtnClick() {
                try {
                    ...
                    getP().addStkCheck(context, staff, stkId, checkTimeStr, wareStr, billId, type, mQueryToken);
                } catch (Exception e) {
                    ToastUtils.showError(e);
                }
            }
    ...
    private boolean isSaveCache = true;//是否保存缓存；提交成功不保存
        ...
        dialog.setOnClickListener(new MyBluetoothScanDialog.OnSuccessListener() {
            ...
            public void onSuccessListener(String result) {
                if (!MyStringUtil.isEmpty(result)) {
                    //扫描结果结尾默认带"\n"
                    if (result.endsWith("\n")) {
                        getP().getWareByScan(context, result.replace("\n", ""), stkId, false);
                    } else {
                        getP().getWareByScan(context, result, stkId, false);
                    }
                }
