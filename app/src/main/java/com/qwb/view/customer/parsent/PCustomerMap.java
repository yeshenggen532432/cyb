...
public class PCustomerMap extends XPresent<CustomerMapActivity> {
    ...
    //查询我的客户
    public void queryData(Activity activity, String memIds, String province, String city, String area, String customerType) {
         Map<String, String> params = new HashMap<>();
        params.put("token", SPUtils.getTK());
        params.put("dataTp", String.valueOf(dataTp));
        params.put("memIds", String.valueOf(memIds));
        ...
        if (MyStringUtil.isNotEmpty(customerType)) {
            params.put("qdtpNm", customerType);
        }
        OkHttpUtils
                .post()
                .params(params)
                .url(Constans.queryCustomerMapList)
                .id(1)
                .build()
    ...
    private void parseJson1(String response) {
        try {
            CustomerMapListResult bean = JSON.parseObject(response, CustomerMapListResult.class);
            if (bean != null && bean.isState()) {
                List<CustomerBean> list = bean.getList();
                getV().doShowCustomerMap(list);
            } else {
