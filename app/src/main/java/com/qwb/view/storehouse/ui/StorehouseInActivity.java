...
public class StorehouseInActivity extends XActivity<PStorehouseIn> {
    ...
    public void initTableView() {
        ...
        mRvLeft.addItemDecoration(new HorizontalDividerItemDecoration.Builder(this)
                ...
                .showLastDivider()
                .build());
        mLeftAdapter = new StorehouseInLeftAdapter();
        ...
        mRvRight.addItemDecoration(new HorizontalDividerItemDecoration.Builder(this)
                ...
                .showLastDivider()
                .build());
        mRightAdapter = new StorehouseInRightAdapter();
        ...
        mRightAdapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                try {
                    StorehouseInSubBean item = (StorehouseInSubBean) adapter.getData().get(position);
                    mCurrentItem = item;
                    ...
                    switch (view.getId()) {
                        ...
                        case R.id.tv_table_content_right_item5://删除
                            showDialogDel();
                            break;
                        case R.id.tv_table_content_right_item6://复制
                            showDialogCopy();
                            break;
                    }
                } catch (Exception e) {
...
//                mHeadRight.setClickable(false);
            ...
            if (MyCollectionUtil.isNotEmpty(subList)) {
                for (StorehouseInSubBean sub : subList) {
            ...
            dialog.setOnOperItemClickL(new OnOperItemClickL() {
                ...
                public void onOperItemClick(AdapterView<?> parent, View view, int i, long id) {
                    ...
                    if(!String.valueOf(mResId).equals(String.valueOf(mStkId))){
                        List<StorehouseInSubBean> list = mRightAdapter.getData();
                        if(MyCollectionUtil.isNotEmpty(list)){
                            for (StorehouseInSubBean subBean:list) {
                                subBean.setInStkId(null);
                                subBean.setInStkName(null);
                            }
                        }
...
//            }
    ...
    public void showDialogCopy(){
        NormalDialog dialog = new NormalDialog(context);
        dialog.content("你确定复制''"+mCurrentItem.getWareNm()+"''吗？").show();
        dialog.setOnBtnClickL(null, new OnBtnClickL() {
            @Override
            public void onBtnClick() {
                try {
                    StorehouseInSubBean subBean = new StorehouseInSubBean();
                    subBean.setId(mCurrentItem.getId());
                    subBean.setWareId(mCurrentItem.getWareId());
                    subBean.setWareNm(mCurrentItem.getWareNm());
                    subBean.setQty(mCurrentItem.getQty());
                    subBean.setInQty1(mCurrentItem.getInQty1());
                    subBean.setBeUnit(mCurrentItem.getBeUnit());
                    subBean.setOldBeUnit(mCurrentItem.getOldBeUnit());
                    subBean.setInStkId(mCurrentItem.getInStkId());
                    subBean.setInStkName(mCurrentItem.getInStkName());
                    subBean.setPrice(mCurrentItem.getPrice());
                    subBean.setAmt(mCurrentItem.getAmt());
                    subBean.setHsNum(mCurrentItem.getHsNum());
                    subBean.setMaxUnitCode(mCurrentItem.getMaxUnitCode());
                    subBean.setMinUnitCode(mCurrentItem.getMinUnitCode());
                    subBean.setWareDw(mCurrentItem.getWareDw());
                    subBean.setMinUnit(mCurrentItem.getMinUnit());

                    mLeftAdapter.getData().add(mCurrentPosition + 1, subBean);
                    mRightAdapter.getData().add(mCurrentPosition + 1, subBean);
                    refreshAdapterRight();
                }catch (Exception e){
                    ToastUtils.showError(e);
                }
    ...
    private void addData() {
        ...
        List<StorehouseInSubBean> subList = mRightAdapter.getData();
        if (MyCollectionUtil.isNotEmpty(subList)) {
            for (StorehouseInSubBean bean : subList) {
                if (bean.getInStkId() == null) {
                    bean.setInStkId(mTempStkId);
                    bean.setInStkName(mTempStkName);
                }
            }
        }
