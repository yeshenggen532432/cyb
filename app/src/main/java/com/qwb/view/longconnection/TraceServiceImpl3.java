 ...
 * 省电模式：
 */
public class TraceServiceImpl3 extends AbsWorkService {
...
//                           "每 3 秒采集一次数据... count = " + count;
                            // 查询数据库最新一条数据与最新定位的时间是否超过60s,超过60s保存插入数据库，否则不插入数据库
                            int traceTime = 20;// 默认一分钟
                            // 在设置中是否设置打开轨迹时间
                            boolean isOPen_traceTime = SPUtils.getBoolean(ConstantUtils.Sp.OPEN_TRACK_CUSTOM_TIME);
                            if (isOPen_traceTime) {
                                int iValues = SPUtils.getIValues(ConstantUtils.Sp.TRACK_CUSTOM_TIME);
                                if (iValues != 0) {
                                    traceTime = iValues/3;
                                }
                            }
//                            if (count > 0 && count % 20 == 0){
                            if (count > 0 && count % traceTime == 0){
...
//            bean.setAddress(mCurrentLocation.getAddrStr());
