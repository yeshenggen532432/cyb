    ...
    public void onClickLocation(View view){
        switch (view.getId()){
            ...
            case R.id.layout_refresh:
                ...
                //地图标注
            case R.id.btn_dtbz:
                ActivityManager.getInstance().jumpToLocationMarkActivity(context, mLatitude, mLongitude, mAddress, null, null, null);
                break;
        }
    }
...
//        });
   ...
   private void  doPlayVoice(){
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // 定位
        if (data != null && requestCode == ConstantUtils.Intent.REQUEST_CODE_LOCATION) {
            mEtCustomerTmp.setText(data.getStringExtra(ConstantUtils.Intent.ADDRESS));
        }
    }
    ...
    private void submitFlow() {
        ...
        String khNm = mEtCustomerTmp.getText().toString().trim();// 位置命名（临时客户）
        getP().addFlow(context, mLongitude, mLatitude, mAddress, bz, mVoiceFile, mVoiceTime, mPicList, khNm);
    }
